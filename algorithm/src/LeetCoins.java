...
//        //0        int num;            //编号
//        //1        int totalCoins;     //该成员所领导团队总币数
//        //2        Link leader;        //领导
//        //3+       List<Link> members; //成员
//
//        maxLength = n + 2;
//        allMember = new int[n][n + 2];
//        for (int i = 0; i < leadership.length; i++) {
//            int leader = leadership[i][0];
//            int leaderIndex = leader - 1;
//            int member = leadership[i][1];
//            int memberIndex = member - 1;
//
//            if (allMember[leaderIndex][0] == 0) {
//                allMember[leaderIndex][0] = leader;
//                //allMember[leaderIndex][1] = 0;
//            }
//            if (allMember[memberIndex][0] == 0) {
//                allMember[memberIndex][0] = member;
//                //allMember[leaderIndex][1] = 0;
//            }
//
//            //添加member
//            for (int j = 3; j < n + 3; j++) {
//                if (allMember[leaderIndex][j] == 0) {
//                    allMember[leaderIndex][j] = member;
//                    break;
//                }
//            }
//            //添加leader
//            allMember[memberIndex][2] = leader;
//        }
//
//        //获取结果数组大小，这个用不了多少时间，也可以用List实现，最后转array，差异不大
//        int resultSize = 0;
...
//        //遍历操作
//        for (int i = 0; i < operations.length; i++) {
//            int memberIndex = operations[i][1] - 1;
//
//            switch (operations[i][0]) {
//                case 1:
//                    allMember[memberIndex][1] += operations[i][2];
//                    addLeader(memberIndex, operations[i][2]);
//                    break;
//                case 2:
//                    int totalCoins = distributeCoin(memberIndex, operations[i][2]);
//                    addLeader(memberIndex, totalCoins);
//                    break;
//                case 3:
//                    result[resultIndex++] = allMember[memberIndex][1] % 1000000007;
//                    break;
//                default:
//            }
//        }
...
//     * 分发币，并统计
//     *
//     * @param leaderIndex 领导下标
//     * @param coins       增加币数
//     * @return 该成员所领导团队增加的总币数
//     */
//    private static int distributeCoin(int leaderIndex, int coins) {
//        int totalCoins = coins;
//
//        if (allMember[leaderIndex][3] != 0) {
//            for (int i = 3; i < maxLength; i++) {
//                if (allMember[leaderIndex][i] == 0) {
//                    break;
//                }
//                int memberCount = distributeCoin(allMember[leaderIndex][i], coins);
//                totalCoins += memberCount;
//            }
//        }
//
//        allMember[leaderIndex][1] += totalCoins;
//        return totalCoins;
//    }
//
//    /**
//     * 给所有上级加数
//     *
//     * @param memberIndex 成员下标
//     * @param totalCoins  该成员所领导团队总币数
//     */
//    private static void addLeader(int memberIndex, int totalCoins) {
//        //member有leader
//        while (allMember[memberIndex][2] != 0) {
//            allMember[allMember[memberIndex][2] - 1][1] += totalCoins;
//            memberIndex = allMember[memberIndex][2] - 1;
//        }
//    }
    ...
    public static int[] bonus(int n, int[][] leadership, int[][] operations) {
        ...
        for (int i = 0; i < leadership.length; i++) {
            ...
            if (links[leaderIndex] == null) {
                links[leaderIndex] = new Link(leader);
            }
            if (links[memberIndex] == null) {
                links[memberIndex] = new Link(member);
            }
            links[leaderIndex].addMember(links[memberIndex]);
            links[memberIndex].leader = links[leaderIndex];
        }
        ...
        for (int i = 0; i < operations.length; i++) {
            ...
            switch (operations[i][0]) {
                case 1:
                    links[memberIndex].totalCoins += operations[i][2];
                    addLeader(links, memberIndex, operations[i][2]);
                    break;
                case 2:
                    int totalCoins = distributeCoin(links, memberIndex, operations[i][2]);
                    addLeader(links, memberIndex, totalCoins);
                    break;
                case 3:
                    result[resultIndex++] = links[memberIndex].totalCoins % 1000000007;
                    break;
                default:
            }
        }
     ...
     * 分发币，并统计
     *
     * @param links       包含所有成员节点的数组
     * @param leaderIndex 领导下标
     * @param coins       增加币数
     * @return 该成员所领导团队增加的总币数
     */
    private static int distributeCoin(Link[] links, int leaderIndex, int coins) {
        int totalCoins = coins;

        Link leaderLink = links[leaderIndex];
        if (leaderLink.members != null) {
            for (Link member : leaderLink.members) {
                totalCoins += distributeCoin(links, member.num - 1, coins);
            }
        }

        links[leaderIndex].totalCoins += totalCoins;
        return totalCoins;
    }
     ...
     * @param totalCoins  该成员所领导团队总币数
     */
    private static void addLeader(Link[] links, int memberIndex, int totalCoins) {
        Link member = links[memberIndex];
        if (member != null) {
            while (member.leader != null) {
                links[member.leader.num - 1].totalCoins += totalCoins;
    ...
    private static class Link {
        int num;            //编号
        List<Link> members; //成员
        Link leader;        //领导
        int totalCoins;     //该成员所领导团队总币数
...
//第一版实现
//HashMap   split ","  Integer.valueOf()  50ms
//HashMap   Link链表                      6ms
//Link[]    Link链表                      3ms
//if 换 switch                            2ms

//    public static int[] bonus(int n, int[][] leadership, int[][] operations) {
//        int[] currentCoins = new int[n];
//
//        Link[] links = new Link[n];
//        for (int i = 0; i < leadership.length; i++) {
//            int leaderIndex = leadership[i][0] - 1;
//            if (links[leaderIndex] == null) {
//                links[leaderIndex] = new Link(leadership[i][1]);
//            } else {
//                Link link = links[leaderIndex];
//                while (link.next != null) {
//                    link = link.next;
//                }
//                link.next = new Link(leadership[i][1]);
//            }
//        }
//
//        int resultSize = 0;
...
//            switch (operations[i][0]) {
//                case 1:
//                    currentCoins[member - 1] += operations[i][2];
//                    break;
//                case 2:
//                    distributeCoin(links, member, operations[i][2], currentCoins);
//                    break;
//                case 3:
//                    result[resultIndex++] = addCoins(links, member, currentCoins);
//                    break;
//                default:
//            }
//        }
...
//    }
//
//    private static void distributeCoin(Link[] links, int leader, int coins, int[] currentCoins) {
//        currentCoins[leader - 1] += coins;
//
//        Link memberLink = links[leader - 1];
//        while (memberLink != null) {
//            distributeCoin(links, memberLink.member, coins, currentCoins);
//            memberLink = memberLink.next;
//        }
//    }
//
//    private static int addCoins(Link[] links, int leader, int[] currentCoins) {
//        int result = currentCoins[leader - 1];
//
//        Link memberLink = links[leader - 1];
//        while (memberLink != null) {
//            result += addCoins(links, memberLink.member, currentCoins);
//            memberLink = memberLink.next;
//        }
//        return result;
//    }
